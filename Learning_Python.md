...
Кортежи в Python — это неизменяемые последовательности, которые могут содержать элементы различных типов. Они похожи на списки, но в отличие от списков, кортежи не могут быть изменены после их создания. Это делает их полезными для хранения данных, которые не должны изменяться.

Создание кортежей
Кортежи создаются с помощью круглых скобок () или просто через запятую. Если кортеж содержит только один элемент, необходимо добавить запятую, чтобы Python распознал его как кортеж.
...
Изменение кортежей
Кортежи неизменяемы, поэтому вы не можете изменять их элементы, добавлять или удалять элементы. Однако вы можете создать новый кортеж на основе существующего.
...
Методы кортежей
Кортежи имеют несколько встроенных методов, хотя их меньше, чем у списков:

count(value): возвращает количество вхождений элемента в кортеже.
...
Кортежи часто используются для хранения данных, которые не должны изменяться, например, координаты, даты и т. д. Они также могут использоваться в качестве ключей в словарях, так как они неизменяемы.

Если у вас есть дополнительные вопросы о кортежах или вы хотите рассмотреть конкретные примеры, дайте знать!

## Глава 5 

Операторы условия if
Оператор if в Python используется для выполнения условных операторов. Он позволяет выполнять определенный блок кода, если заданное условие истинно. Если условие ложно, можно использовать дополнительные операторы elif (сокращение от "else if") и else для выполнения других блоков кода.
...
```
Несколько условий:
Вы можете использовать логические операторы, такие как and, or, и not, для комбинирования условий.
...
Вы можете использовать операторы if внутри других операторов if.
...
Вот пример, который использует ввод от пользователя:
...
if age < 18:
    ...
    print("Вы на пенсии.")
Заключение
Оператор if в Python — это мощный инструмент для управления потоком выполнения программы на основе условий. Он позволяет создавать логические ветвления и принимать решения в коде. Если у вас есть дополнительные вопросы или вы хотите рассмотреть конкретные примеры, дайте знать!
